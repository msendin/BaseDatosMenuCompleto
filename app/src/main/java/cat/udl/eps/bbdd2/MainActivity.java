...
public class MainActivity extends Activity {
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		btnInsertar.setOnClickListener(v -> {
			...
			long cont = db.insert("Usuarios", null, nuevoRegistro);
			if (cont > 0)
				showToast(getString(R.string.correctIn) + cont);
			else
		...
		btnActualizar.setOnClickListener(v -> {
			...
			if (cont > 0)
				showToast(getString(R.string.correctAct)  + cont);
			else
		...
		btnEliminar.setOnClickListener(v -> {
			...
			if (cont > 0)
				showToast(getString(R.string.correctEl) + cont);
			else
				showToast(getString(R.string.noReg));
		});
